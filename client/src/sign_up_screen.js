...
export default function RegistroScreen({ navigation }) {
  ...
  async function otorgarPermisos(password) {
      const response = await fetch('http://192.168.0.2:8080/otorgarPermisos', {
          method: 'POST',
          headers: {
              'Content-Type': 'application/json',
          },
  ...
  async function signUp(nombre, correo, password, foto_usuario, permisos, latitud, longitud ) {
    const response = await fetch('http://192.168.0.2:8080/aniadirUsuario', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
    ...
    const data = { id_usuario, nombre, correo, password, foto_usuario, permisos, latitud, longitud }
    navigation.replace("Index", data)
  }

  const recopilarCampos = async () => {
    const permisos = await otorgarPermisos(c_contrasena)
    const disponible = await verificarCorreo(c_correo)

    if ( disponible === true ) {
      signUp(n_usuario, c_correo, c_contrasena, i_uri, permisos, 0, 0)
    } else {
      setTError("Correo ya registrado")
    }
  }

  const v_validaCampos = () => {
    if (!i_uri) {
      setTError("Por favor selecciona una imagen de perfil.");
      return false;
    }
  ...
  const b_registrar = async () => {
    if (v_validaCampos()) {
      await recopilarCampos()
  ...
  return (
    ...
    >
      <View style={styles.v_cabecera}>
        ...
        <Text style={styles.t_titulo}>Follow Pet</Text>
